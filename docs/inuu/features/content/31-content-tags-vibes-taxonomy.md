...
- [x] В БД ≥40 тегов из мастер-списка для активного города.
- [x] Groq возвращает только slug из whitelist (или `new_tags` в moderation).
- [x] Афиша: фильтр по новым тегам работает (`?tag=romance`).
- [x] Документ в промпте и runbook менеджера синхронизированы.
