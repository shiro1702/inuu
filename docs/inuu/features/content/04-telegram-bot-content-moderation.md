...
После публикации — обновить карточку: убрать кнопки approve/reject, дописать строку со **ссылкой на событие на сайте** (`https://…/{city_slug}/events/{slug}` из `NUXT_APP_URL`) и показать клавиатуру ⭐1–5:

```
✅ Опубликовано на сайте
https://inuu.example/ulan-ude/events/goncharnyj-krug

Оцените приоритет в ленте:
[ ⭐1 ] … [ ⭐5 ]
```

После оценки ссылка на сайт остаётся в тексте карточки; в футере — `@editor` и дата.
