# Stateless-мост Web → TMA (корзина еды, legacy)

> **Архив.** Передача корзины блюд через `startapp`. INUU не использует; booking — с сервера / сессии.

Документ описывает **безБД‑подход** для передачи корзины
