...
function removeParserChat(channel: 'telegram' | 'max', value: string) {
  if (channel === 'telegram') {
    settingsForm.telegramParserSourceChats = settingsForm.telegramParserSourceChats.filter((x) => x !== value)
    return
  }
  settingsForm.maxParserSourceChats = settingsForm.maxParserSourceChats.filter((x) => x !== value)
}
