<template>
  <div>
    ...
    <p class="mt-2 text-sm text-gray-600">Заведения и точки в {{ displayName }}</p>
    ...
    <div v-else-if="items.length" class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      <CityVenueCard v-for="venue in items" :key="venue.id" :venue="venue" />
    </div>
    <p v-else class="mt-8 text-sm text-gray-500">Пока нет опубликованных мест.</p>
  </div>
</template>
