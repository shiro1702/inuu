      ...
      if (branch.value) {
        ...
        if (branch.value && storefrontPath.value) {
          _push(ssrRenderComponent(_component_NuxtLink, {
            ...
            default: withCtx((_, _push2, _parent2, _scopeId) => {
              if (_push2) {
                _push2(` Витрина · этот филиал `);
              } else {
                return [
                  createTextVNode(" Витрина · этот филиал ")
                ];
              }
            }),
            _: 1
        ...
        if (branch.value && storefrontPath.value) {
          ...
          ssrRenderList(tables.value, (table) => {
            ...
            _push(`</div><div class="min-w-0 flex-1"><p class="text-sm font-medium text-gray-900">Столик №${ssrInterpolate(table.tableNumber)}</p><p class="break-all text-xs text-gray-500">${ssrInterpolate(buildTableQrUrl())}</p></div><div class="flex items-center gap-2"><button type="button" class="rounded-lg border border-gray-300 px-2.5 py-1.5 text-xs hover:bg-gray-100 disabled:opacity-50"${ssrIncludeBooleanAttr(!buildTableQrUrl()) ? " disabled" : ""}>${ssrInterpolate(copyFeedbackByTableId.value[table.id] || "Скопировать QR-ссылку")}</button><button type="button" class="${ssrRenderClass([table.isActive ? "border-amber-300 text-amber-700 hover:bg-amber-50" : "border-emerald-300 text-emerald-700 hover:bg-emerald-50", "rounded-lg border px-2.5 py-1.5 text-xs disabled:opacity-50"])}"${ssrIncludeBooleanAttr(!canEditCritical.value || updatingTableId.value === table.id) ? " disabled" : ""}>${ssrInterpolate(table.isActive ? "Деактивировать" : "Активировать")}</button></div></div>`);
          });
          _push(`<!--]-->`);
