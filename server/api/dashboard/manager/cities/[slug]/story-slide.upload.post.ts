...
function sanitizeFileName(input: string): string {
  const cleaned = input.replace(/[^a-zA-Z0-9._-]/g, '_').slice(-80) || 'story-slide'
...
export default defineEventHandler(async (event) => {
  ...
  const campaignPart = body.campaignId ? `${body.campaignId}/` : ''
  const sortPart = typeof body.sortOrder === 'number' ? `s${body.sortOrder}-` : ''
  const objectPath = `inuu-content/${scope.cityId}/stories/${campaignPart}${sortPart}${Date.now()}-${safeName}.${ext}`
